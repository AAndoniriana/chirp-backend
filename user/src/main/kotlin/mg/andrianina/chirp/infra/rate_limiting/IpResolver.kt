...
import mg.andrianina.chirp.infra.config.NginxConfig
        ...
        return try {
            val inetAddress = when {
                ...
                trimmedIp.matches(Regex("\\d+\\.\\d+\\.\\d+\\.\\d+")) ->
                    ...
                    logger.warn("Invalid IP format in $headerName: $ip from $proxyIp")
                     return null
                }
            }

            if (isPrivateIp(inetAddress.hostAddress)) {
                logger.debug("Private IP address in $headerName: $trimmedIp from $proxyIp")
            }
